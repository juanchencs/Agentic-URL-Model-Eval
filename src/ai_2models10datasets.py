...
def read_wiki_credentials(wiki_file: Path) -> tuple[str, str]:
    ...
    if not token:
        if len(lines) >= 2 and "@" in lines[0]:
            ...
            token = lines[0]
    if not token:
        raise ValueError("Could not parse Confluence token from wiki_jane.txt")
    return email, token
...
def run_pipeline(
    ...
    wiki_token_file: str = "/home/ubuntu/efs/urlmodel/wiki_jane.txt",
) -> dict[str, Any]:
    ...
    if confluence_publish:
        try:
            ...
            confluence_error = (
                ...
                f"Check wiki_jane token permissions for space '{confluence_space_key}'. "
                f"Response: {body}"
            )
...
def main(CONFLUENCE_PUBLISH: bool = False) -> None:
    MODEL_VERSION_NEW = "20250101"  # alias: MLP1, keep private, 20250101 is not the actual model version
    MODEL_VERSION_OLD = "20240101"  # alias: ML, keep private, 20240101 is not the actual model version
    ...
    WIKI_TOKEN_FILE = "/home/ubuntu/efs/urlmodel/wiki_jane.txt" #keep private, not the actual token file path
